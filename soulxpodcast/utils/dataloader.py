...
# The official ONNX Runtime wheels enable telemetry by default on Linux/macOS.
# This must be set before importing onnxruntime so no uploader or persistent
# device identifier is initialized.
...
try:
    ...
    # Some CPU-only distributions ship a torchaudio extension linked against
    # CUDA. The project only needs basic loading, resampling, and 80-bin
    # speaker features, so keep a librosa/soundfile fallback for Mac/CPU.
    torchaudio = None
    kaldi = None
...
def _load_audio(path: str, sample_rate: int) -> tuple[torch.Tensor, int]:
    """Load mono float audio without requiring a CUDA-linked torchaudio."""
    if TORCHAUDIO_AVAILABLE:
        audio, source_rate = torchaudio.load(path, backend="soundfile")
    ...
    audio_np, source_rate = sf.read(path, always_2d=False)
    if audio_np.ndim > 1:
        audio_np = audio_np.mean(axis=1)
    audio_np = audio_np.astype("float32", copy=False)
    if source_rate != sample_rate:
        audio_np = librosa.resample(
            audio_np, orig_sr=source_rate, target_sr=sample_rate
        ).astype("float32", copy=False)
    return torch.from_numpy(audio_np).unsqueeze(0), sample_rate


def _speaker_fbank(audio: torch.Tensor, sample_rate: int = 16000) -> torch.Tensor:
    """Create the 80-bin speaker feature expected by CAMPPlus."""
    if TORCHAUDIO_AVAILABLE:
        return kaldi.fbank(
            audio, num_mel_bins=80, dither=0, sample_frequency=sample_rate
        )
    ...
    return torch.from_numpy(np.log(np.maximum(mel, 1e-10)).T.astype("float32"))
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            for spk_idx, (prompt_text, prompt_wav) in enumerate(zip(data["prompt_text"], data["prompt_wav"])):
                ...
                spk_feat = _speaker_fbank(audio.unsqueeze(0), sample_rate=16000)
                ...
                # 3. feature for flow
                audio, sample_rate = _load_audio(prompt_wav, sample_rate=24000)
                audio = audio_volume_normalize(audio.squeeze(0)).unsqueeze(0)
