...
from soulxpodcast.utils.device import autocast_context, get_device
...
class SoulXPodcast(torch.nn.Module):
    def __init__(self, config: Config = None):
        ...
        if self.config.hf_config.fp16_flow and self.device.type == "cuda":
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S,%f')[:-3]
            tqdm.write(f"[{timestamp}] - [INFO] - Casting flow to fp16")
            self.flow.half()
        elif self.config.hf_config.fp16_flow:
            timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S,%f')[:-3]
            tqdm.write(
                f"[{timestamp}] - [WARNING] - fp16_flow is CUDA-only; using portable precision on {self.device}"
            )
        ...
        **kwargs,  # for compatibility
        ...
        for i in range(turn_size):
            ...
            flow_input = torch.tensor(
                [prompt_speech_token + generated_speech_tokens], device=self.device
            )
            flow_inputs_len = torch.tensor(
                [len(prompt_speech_token) + len(generated_speech_tokens)], device=self.device
            )
            ...
            # Flow generation
            with autocast_context(self.device, self.config.hf_config.fp16_flow):
